...
const GROUP_BY_KEY_TO_COUNTS_MATERIALIZED_VIEW = {
    organism: 'obelisk_virome_counts_sra_organism',
    bioproject: 'obelisk_virome_counts_sra_bioproject',
    assay_type: 'obelisk_virome_counts_sra_assay_type',
    tax_species: 'obelisk_virome_counts_palm_virome_tax_species',
    tax_family: 'obelisk_virome_counts_palm_virome_tax_family',
    sotu: 'obelisk_virome_counts_palm_virome_sotu',
    tissue: 'obelisk_virome_counts_biosample_tissue',
    geo_attribute_value: 'obelisk_virome_counts_biosample_geographical_location',
    biome_attribute_value: 'obelisk_virome_counts_bgl_gm4326_gp4326',
    stat_host_order: 'obelisk_virome_counts_sra_stat',
    sex: 'obelisk_virome_counts_biosample_sex',
    do_label: 'obelisk_virome_counts_biosample_disease',
    community_id: 'obelisk_virome_counts_virome_community',
};
...
export const getMinimalJoinSubQuery = (filters, palmprintOnly, groupBy = undefined) => {
    ...
    tables = ['obelisk_virome_identifiers', ...tables.filter((table) => table !== 'obelisk_virome_identifiers')];
    ...
    const buildJoinStatement = (table, index, selectStatement, whereStatement, tableJoinKey) => {
        if (index === 0) {
            ...
            return `INNER JOIN (
                ...
                FROM ${table}
                ${whereStatement}
            ) as ${table} ON obelisk_virome_identifiers.${tableJoinKey} = ${table}.${tableJoinKey}`;
        }
    };
    ...
    const selectStatements = [
        'obelisk_virome_identifiers.run_id as run_id, obelisk_virome_identifiers.biosample as biosample, obelisk_virome_identifiers.bioproject as bioproject',
    ];
